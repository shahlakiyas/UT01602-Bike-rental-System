...
const NotificationURl = "http://localhost:5263/api/RentalRequest/Get-Notifiactions";

let notifications = document.getElementById('notifications');
console.log(notifications);


let NotifyUser = JSON.parse(sessionStorage.getItem('currentUser'));
if(NotifyUser){
  console.log(NotifyUser)
  notifications.addEventListener('mouseout' ,()=> getUserNotifications(NotifyUser.nicNumber))
}



async function getUserNotifications(NICNo){

  let noticationBar = document.getElementById('noticationBar');
  noticationBar.innerHTML = "";
  const response = await fetch(`${NotificationURl}${NICNo}`);
  if (response.ok) {
    ...
    data.forEach(notifiaction => {
      ...
      if(notifiaction.status == true){
        info.innerText = `Your request for ${notifiaction.requestTime} has been accepted`
      }else if(notifiaction.status == false){
        info.innerText = `Sorry...Your request for ${notifiaction.requestTime} has been declined`
      }
      noticationBar.append(info);
    });
   // return data;
  } else {
